...
######### 
setwd("~/Dropbox (UC Berkeley Biostat)/hubbardlap/Mitch Cohen/Lucy/Results/SL Results/")
no=length(out)
for( i in 1:no) {
for(j in 1:ns) {
outc=out[i]
...
fit.test=try(CV.SuperLearner(Y,X,family=binomial(),SL.library=SL.library,V=V,verbose=F),silent=T)
if(length(fit.test)>1) {
##### ROC Curve
fld=fit.test$fold
YY=fit.test$Y
...
ciout=try(ci.cvAUC(predsY, YY, folds = fold),silent=T)
if(length(ciout)> 1) {
...
write.table(outtab, file="SLAucRes.csv",sep=",",quote = TRUE,na=".",row.names = FALSE)
