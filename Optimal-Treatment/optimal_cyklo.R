...
dataf=read.dta13("/Users/waverlywei/Desktop/Lucy project/ACIT_6Jan17_deidentified1.dta")
...
V = dataf %>% 
  select(sample0h_ex_ml, sample0h_crt_ly30, 
         sample0h_ck_ly30, sample0h_ck_ly60) %>% 
  mutate(sample0h_ck_ly60=as.numeric(sample0h_ck_ly60))
...
Vnorm <- final_V
Vnorm  <- Vnorm %>% 
  mutate(sample0h_ex_ml=as.numeric((sample0h_ex_ml<15))) %>% 
  mutate(sample0h_crt_ly30=as.numeric((sample0h_crt_ly30>=0) & (sample0h_crt_ly30<=3))) %>% 
  mutate(sample0h_ck_ly30=as.numeric((sample0h_ck_ly30>=0) & (sample0h_ck_ly30<=8))) %>% 
  mutate(sample0h_ck_ly60=as.numeric((sample0h_ck_ly60>=0) & (sample0h_ck_ly60<=15))) 


A <- dataf %>% 
  select(cyklokaproninpt)%>% 
  mutate(cyklokaproninpt = as.numeric(cyklokaproninpt>0))
...
V_nam <- c("V1","V2","V3","V4","V5","V6","V7","V8")
all <- all[,c(-1,-18,-29)]
names(all) <- c(W_nam,V_nam,"A","Y")
all <- all %>% drop_na()

# NO observations left after removing NA's 
