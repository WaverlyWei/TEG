...
## IMPUTATION
imp_V <- amelia(V)
write.amelia(imp_V, file.stem <- "Desktop/Lucy project/imputed_data_set_V")
complete_V <- read.csv("Desktop/Lucy project/imputed_data_set_V1.csv")
...
## CODE A
A <- dataf %>% 
  select(icu_0to6h_ffp_units)%>% 
  mutate(icu_0to6h_ffp_units = as.numeric(icu_0to6h_ffp_units>0))
