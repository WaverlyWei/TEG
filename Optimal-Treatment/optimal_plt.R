...
dataf=read.dta13("Desktop/Lucy project/ACIT_6Jan17_deidentified1.dta")
...
V <- dataf %>% select(sample0h_ex_a10, sample0h_ex_a20, 
                     sample0h_ex_mcf, sample0h_crt_ma, sample0h_ck_ma)
...
## IMPUTATION
imp_V <- amelia(V)
write.amelia(imp_V, file.stem ="Desktop/Lucy project/imputed_data_plt_V")
complete_V <- read.csv("Desktop/Lucy project/imputed_data_plt_V1.csv")
...
Vnorm <- final_V
Vnorm  <- Vnorm %>% 
  mutate(sample0h_ex_a10=as.numeric((sample0h_ex_a10>=43) & (sample0h_ex_a10<=65))) %>% 
  mutate(sample0h_ex_a20=as.numeric((sample0h_ex_a20>=50) & (sample0h_ex_a20<=71))) %>% 
  mutate(sample0h_ex_mcf=as.numeric((sample0h_ex_mcf>=50) & (sample0h_ex_mcf<=72))) %>% 
  mutate(sample0h_crt_ma=as.numeric((sample0h_crt_ma>=52) & (sample0h_crt_ma<=72))) %>%
  mutate(sample0h_ck_ma=as.numeric((sample0h_ck_ma>=51) & (sample0h_ck_ma<=69))) 


## CODE A
A <- dataf %>% 
  select(icu_0to6h_plt_units)%>% 
  mutate(icu_0to6h_plt_units = as.numeric(icu_0to6h_plt_units>0))
...
V_nam <- c("V1","V2","V3","V4","V5","V6","V7","V8","V9","V10")
all <- all[,c(-1,-18,-31)]
